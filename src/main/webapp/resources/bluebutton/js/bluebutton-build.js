...
Core.XML = (function () {
  ...
  var attr = function (attr) {
    ...
    var attrVal = this.el.getAttribute(attr);
    return attrVal || null;
  };
  ...
  var val = function () {
    ...
    if (!textContent && this.el.childNodes.length === 1 &&
          this.el.childNodes[0].tagName === 'reference') {

      var contentId = this.el.childNodes[0].getAttribute('value');
...
Documents.HEALTHVAULT = (function () {
  ...
  var section = function (name) {
    ...
    switch (name) {
      case 'document':
        return this.template('2.16.840.1.113883.10.20.1');
      case 'allergies':
...
/*
 * Parser for the C32 labs section
 */

Parsers.C32.labs = function (c32) {
...
Parsers.C32.medications = function (c32) {
  ...
  medications.entries().each(function(entry) {
    ...
    if (!route_name) {
        el = entry.tag('routeCode').tag('originalText');
        if (!el.isEmpty()) {
        	route_name = Core.stripWhitespace(el.val());
        }
      }
    // participant => vehicle
    el = entry.tag('participant').tag('code');
...
Parsers.HEALTHVAULT = (function () {
  ...
  var run = function (ccda) {
    ...
    data.problems             = Parsers.HEALTHVAULT.problems(ccda);
...
/*
 * Parser for the HEALTHVAULT allergies section
 */

Parsers.HEALTHVAULT.allergies = function (ccda) {
  ...
  allergies.entries().each(function(entry) {
    ...
    el = entry.template('2.16.840.1.113883.10.20.1.18').tag('code');
    var name = el.attr('displayName'),
        code = el.attr('code'),
        code_system = el.attr('codeSystem'),
        code_system_name = el.attr('codeSystemName');
    ...
    // value => reaction_type
    el = entry.template('2.16.840.1.113883.10.20.1.18').tag('value');
    var reaction_type_name = el.attr('displayName'),
        reaction_type_code = el.attr('code'),
        reaction_type_code_system = el.attr('codeSystem'),
        reaction_type_code_system_name = el.attr('codeSystemName');
...
/*
 * Parser for the HEALTHVAULT demographics section
 */

Parsers.HEALTHVAULT.demographics = function (ccda) {
  ...
  el = patient.tag('patient');
  var dob = parseDate(el.tag('birthTime').attr('value')),
      gender = Core.Codes.gender(el.tag('administrativeGenderCode').tag('originalText').val().toUpperCase()),
      marital_status = Core.Codes.maritalStatus(el.tag('maritalStatusCode').attr('code'));
...
/*
 * Parser for the HEALTHVAULT immunizations section
 */

Parsers.HEALTHVAULT.immunizations = function (ccda) {
...
/*
 * Parser for the HEALTHVAULT problems section
 */

Parsers.HEALTHVAULT.problems = function (ccda) {
...
var BlueButton = function (source, opts) {
  ...
  if (opts.parser) {
    ...
    switch (type) {
      case 'healthvault':
    	  parsedData = Documents.HEALTHVAULT.process(parsedData);
    	  parsedDocument = Parsers.HEALTHVAULT.run(parsedData);
    	//  parsedDocument = Parsers.HEALTHVAULT.run(parsedData);
    	  break;
      case 'c32':
