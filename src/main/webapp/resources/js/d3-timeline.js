    ...
    function timeline (gParent) {
      ...
      g.each(function(d, i) {
        d.forEach( function(datum, index){
          ...
          g.selectAll("svg").data(data).enter()
            .append((displayShape) ? displayShape(d, i,datum) : display)
            .attr("x", getXPos)
            .attr("y", getStackPosition)
            .attr("width", function (d, i) {
            	if(d.ending_time == d.starting_time){return 0;}
            	var startTimeRect = (d.starting_time < beginning) ? beginning : d.starting_time;
            	var endTimeRect = (d.ending_time > ending) ? ending : d.ending_time;
            	return (endTimeRect -startTimeRect)*scaleFactor;
            })
          ...
          function getStackPosition(d, i) {
            if (stacked) {
              var displayShapeType = (displayShape) ? displayShape(d, i,datum) : display;
              if (displayShapeType === "circle"){ 
            	  return (margin.top + (itemHeight + itemMargin) * yAxisMapping[index]) + (itemHeight /2);
              }
              else {
...
//          .call(zoom);
      ...
      function getXPos(d, i) {
        return margin.left + ((d.starting_time >= beginning) ?  d.starting_time - beginning : 0) * scaleFactor;
      }
      ...
      function setHeight() {
        if (!height && !gParentItem.attr("height")) {
          if (itemHeight) {
            ...
            height = (height == 0) ? (margin.top + ((itemHeight + itemMargin) * maxStack) + (showBorderFormat.marginTop - showBorderFormat.marginBottom)) : height;
    ...
    timeline.onclick = function (titleFunc) {
    ...
    timeline.displayShape = function (titleFunc) {
          if (!arguments.length) return displayShape;
          displayShape = titleFunc;
          return timeline;
        };
